...
def linkage_tree(X, connectivity=None, n_components=None,
    ...
    # Jesse: after final merge, cease to yield further results
...
class AgglomerativeClustering(BaseEstimator, ClusterMixin):
    ...
    def __init__(self, n_clusters=2, affinity="euclidean",
                 ...
                 pooling_func=np.mean, max_size=sys.maxint,
                 means=None, variances=None):

        # print "hac: __init__ called"  # DEBUG
        ...
        # Jesse: new keyword arguments
        self.max_size = max_size
        self.means = means[:] if means is not None else None
        self.variances = variances[:] if variances is not None else None

        # Jesse: added these declarations to allow direct access from outside class
        self.children_ = None
    ...
    def fit(self, X, y=None):
        ...
        # Jesse - convert to generator expression to get results after each k merge
        # Jesse - removed n_clusters argument to default to None, which makes alg run in generator fashion
        kwargs['max_size'] = self.max_size
        kwargs['means'] = self.means
        kwargs['variances'] = self.variances
        for params in memory.cache(tree_builder)(X, connectivity, n_components=self.n_components, **kwargs):
            self.children_, self.n_components_, self.n_leaves_, parents = params

            # print "hac: got result yielded from tree_builder"  # DEBUG

            # Jesse - often stopping before computing full tree
            # Cut the tree
            # if compute_full_tree:
