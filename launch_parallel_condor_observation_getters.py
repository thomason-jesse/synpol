...
def main():
    ...
    for jdx in range(0, FLAGS_jobs_to_launch):
        ...
        # create pickle containing subset of information
        job_wnids = wnids[int(head_wnid):int(tail_wnid)]
        job_synsets = synsets[int(head_wnid):int(tail_wnid)]
        fn = "condor_temp.pickle."+str(int(head_wnid))+"-"+str(int(tail_wnid))
        f = open(fn, 'wb')
        pickle.dump([job_wnids, job_synsets, noun_phrases, polysems], f)
        f.close()

        # launch a condor job to process just this pickle
